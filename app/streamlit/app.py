...
# Configure matplotlib for English / Unicode rendering.
# Architectural decision: Plot axes, labels, and titles strictly use standard scientific English
# to guarantee 100% robust cross-platform rendering without CJK 'Tofu' glyph missing errors in Docker/Linux.
plt.rcParams['font.sans-serif'] = ['DejaVu Sans', 'Arial', 'Microsoft YaHei', 'sans-serif']
...
from app.streamlit.theme import render_hero_section

# Initialize i18n with default language en_US
...
st.set_page_config(
    page_title=_("NPS Spectral Intelligence Platform"),
    page_icon="🧪",
    layout="wide",
...
# Top Hero & Workflow Instructions
render_hero_section(
...
MODEL_OPTIONS = {
    "binary": "Binary Model (SC versus NSC)",
    "multi": "Multi-class Model (nine NPS categories)",
}

model_type_key = st.sidebar.radio(
    _("Select Model"),
    options=["binary", "multi"],
    format_func=lambda k: _(MODEL_OPTIONS[k]),
    index=0
)
...
if uploaded_file is not None:
    ...
    with st.spinner(f"Processing {filename}..."):
        try:
            ...
            # Record privacy and contributions

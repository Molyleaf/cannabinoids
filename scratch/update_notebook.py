import json
from pathlib import Path

def update_notebook():
    nb_path = Path("simclr_finetune") / "预训练+二分类.ipynb"
    print(f"Reading notebook: {nb_path}")

    with open(nb_path, "r", encoding="utf-8") as f:
        nb = json.load(f)

    updated = False
    for idx, cell in enumerate(nb["cells"]):
        source_text = "".join(cell.get("source", []))
        if "class SpectrumEncoder" in source_text:
            print(f"Found SpectrumEncoder in cell {idx}, updating architecture...")
            
            # 将 cell 的 source 分行处理并替换
            new_source_lines = [
                '"""\n',
                '大麻素 vs 非大麻素 二分类（阳性按SMILES分组，阴性随机划分）\n',
                '使用预训练模型: best_model_0716.pt\n',
                '文件: binary_classification_with_best_model.py\n',
                '"""\n',
                '\n',
                'import torch\n',
                'import torch.nn as nn\n',
                'import torch.nn.functional as F\n',
                'from torch.utils.data import DataLoader, TensorDataset\n',
                'import numpy as np\n',
                'import pickle\n',
                'import matplotlib.pyplot as plt\n',
                'from pathlib import Path\n',
                'from tqdm import tqdm\n',
                'from collections import Counter, defaultdict\n',
                'from sklearn.metrics import (\n',
                '    classification_report, confusion_matrix,\n',
                '    roc_auc_score, roc_curve, accuracy_score,\n',
                '    precision_score, recall_score, f1_score\n',
                ')\n',
                'import seaborn as sns\n',
                'import warnings\n',
                'import pandas as pd\n',
                'from datetime import datetime\n',
                "warnings.filterwarnings('ignore')\n",
                '\n',
                '\n',
                '# ==================== 编码器 ====================\n',
                '\n',
                'class SpectrumEncoder(nn.Module):\n',
                '    def __init__(self, input_dim=561, hidden_dim=256):\n',
                '        super().__init__()\n',
                '        self.conv_block = nn.Sequential(\n',
                '            nn.Conv1d(1, 64, kernel_size=7, padding=3),\n',
                '            nn.GroupNorm(8, 64),\n',
                '            nn.ReLU(inplace=True),\n',
                '            nn.MaxPool1d(2),\n',
                '            nn.Conv1d(64, 128, kernel_size=5, padding=2),\n',
                '            nn.GroupNorm(8, 128),\n',
                '            nn.ReLU(inplace=True),\n',
                '            nn.MaxPool1d(2),\n',
                '            nn.Conv1d(128, 256, kernel_size=3, padding=1),\n',
                '            nn.GroupNorm(8, 256),\n',
                '            nn.ReLU(inplace=True),\n',
                '            nn.AdaptiveAvgPool1d(1)\n',
                '        )\n',
                '        self.fc = nn.Sequential(\n',
                '            nn.Linear(256, hidden_dim),\n',
                '            nn.LayerNorm(hidden_dim)\n',
                '        )\n',
                '    \n',
                '    def forward(self, x):\n',
                '        if x.dim() == 2:\n',
                '            x = x.unsqueeze(1)\n',
                '        h = self.conv_block(x).squeeze(-1)\n',
                '        embed = self.fc(h)\n',
                '        norm = torch.norm(embed, p=2, dim=1, keepdim=True)\n',
                '        return embed / torch.clamp(norm, min=1e-3)\n',
                '\n',
                '\n',
                'class BinaryClassifier(nn.Module):\n',
                '    """二分类头"""\n',
                '    def __init__(self, encoder, input_dim=256, freeze_encoder=True):\n',
                '        super().__init__()\n',
                '        self.encoder = encoder\n',
                '        \n',
                '        if freeze_encoder:\n',
                '            for param in self.encoder.parameters():\n',
                '                param.requires_grad = False\n',
                '        \n',
                '        self.classifier = nn.Sequential(\n',
                '            nn.Linear(input_dim, 128),\n',
                '            nn.LayerNorm(128),\n',
                '            nn.ReLU(inplace=True),\n',
                '            nn.Dropout(0.3),\n',
                '            nn.Linear(128, 64),\n',
                '            nn.LayerNorm(64),\n',
                '            nn.ReLU(inplace=True),\n',
                '            nn.Dropout(0.2),\n',
                '            nn.Linear(64, 1)\n',
                '        )\n',
                '    \n',
                '    def forward(self, x):\n',
                '        with torch.set_grad_enabled(not all(p.requires_grad == False \n',
                '                                            for p in self.encoder.parameters())):\n',
                '            embed = self.encoder(x)\n',
                '        logit = self.classifier(embed)\n',
                '        return logit.squeeze(-1)\n',
                '\n',
                '\n'
            ]
            cell["source"] = new_source_lines
            updated = True

    if updated:
        with open(nb_path, "w", encoding="utf-8") as f:
            json.dump(nb, f, ensure_ascii=False, indent=1)
        print("Successfully updated notebook architecture!")
    else:
        print("No matching cell found.")

if __name__ == "__main__":
    update_notebook()

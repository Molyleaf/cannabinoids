...
def parse_large_msp_generator(file_path):
    """
    流式读取 MSP 文件，以生成器形式返回每一个谱图块的 string
    """
    current_block = []
...
def evaluate_msp_file(msp_path, dataset_name):
    ...
    print(f"  风险概率分位数       : 25%={stats['p25']:.4f}, 75%={stats['p75']:.4f}, 90%={stats['p90']:.4f}, 95%={stats['p95']:.4f}, 99%={stats['p99']:.4f}")
    print(f"  最小/最大风险概率   : {stats['min_prob']:.4f} / {stats['max_prob']:.4f}")
    
    print(f"\n--- {dataset_name} 风险概率最高的样本 ---")
    top_high = df.sort_values(by="Model Risk Prob", ascending=False).head(20)
    print(top_high[["Index", "Name", "Known Lib Matched", "Model Risk Prob"]].to_string(index=False))
    
    return stats, df

def main():
    neg3_msp = os.path.join(project_root, "scratch", "阴性(3).msp")
    if os.path.exists(neg3_msp):
        stats, df = evaluate_msp_file(neg3_msp, "阴性(3).msp")
        if df is not None:
            df.to_csv(os.path.join(project_root, "scratch", "evaluation_results_neg3.csv"), index=False, encoding="utf-8-sig")
            with open(os.path.join(project_root, "scratch", "evaluation_summary_neg3.json"), "w", encoding="utf-8") as f:
                json.dump(stats, f, indent=2, ensure_ascii=False)
            print("\n评估完成，结果保存至 scratch/evaluation_results_neg3.csv 及 evaluation_summary_neg3.json")
    else:
        print(f"文件不存在: {neg3_msp}")
